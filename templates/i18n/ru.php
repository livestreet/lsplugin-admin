...
return array(
	...
	'errors' => array(
		 ...
		 */
		'profile_edit' => array(
			'wrong_user_id' => 'Пользователь с таким ид не найден',
		...
		'encoding_check' => array(
			...
			'utf8_bom_encoding_detected' => 'В файлах, указанных ниже, найдена некорректная кодировка UTF-8 BOM,
			которая может вызывать разные проблемы при работе с движком, нужно открыть указанные файлы и изменить их кодировку на "UTF-8 <i>БЕЗ</i> BOM"',
			'utf8_bom_file' => 'Файл',
		),
	...
	'skin' => array(
		...
		'this_is_preview' => 'Для вас включен предпросмотр выбранного шаблона. Откройте главную страницу сайта и рассмотрите шаблон.',
	...
	'utils' => array(
		...
		'files' => array(
			...
			'info' => 'Данный раздел позволяет проверить наиболее часто редактируемые файлы вашего сайта в поисках кодировки,
			которая крайне не рекомендована для использования - UTF-8 BOM. Если такие файлы будут найдены, то будет выведен их список.
			Список файлов (маски путей) задаются в конфиге админки',

...
{block name='layout_content_actionbar'}
	<div class="fl-r">
		<a class="button {if $_aRequest.type==''}active{/if}"
		   href="{router page='admin/plugins/list'}">{$aLang.plugin.admin.plugins.menu.filter.activated} ({$aPluginsInfo.count})</a>
		<a class="button {if $_aRequest.type=='deactivated'}active{/if}"
		   href="{router page='admin/plugins/list'}?type=deactivated">{$aLang.plugin.admin.plugins.menu.filter.deactivated} ({$aPluginsInfo.count_all - $aPluginsInfo.count})</a>

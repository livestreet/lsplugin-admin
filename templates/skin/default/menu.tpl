{foreach from=$oMenuMain->GetSections() item=oMenuSection key=section_index}
	<br/>
	<a
		{if !$oMenuSection->HasItems()}href="{$oMenuSection->GetUrlFull()}"{/if}
		class="sb-item {$oMenuSection->GetCssClass()}
			{if $oMenuSection->HasItems()}sb-root{/if}
			{if $oMenuSection->GetActive()}active{/if}"
		id="sb-root-{$oMenuMain->GetName()}-{$section_index}"
		><span>{$oMenuSection->GetCaption()|escape:'html'}</span></a>


	{if $oMenuSection->HasItems()}
	<ul>
	{foreach from=$oMenuSection->GetItems() item=oMenuItem}
		<li{if $oMenuItem->GetActive()} class="active"{/if}><a
			href="{$oMenuItem->GetUrlFull()}"
			>{$oMenuItem->GetCaption()|escape:'html'}</a></li>
	{/foreach}
	</ul>
	{/if}
{/foreach}

<div class="sb-sep">{$oMenuAddition->GetCaption()|escape:'html'}</div>

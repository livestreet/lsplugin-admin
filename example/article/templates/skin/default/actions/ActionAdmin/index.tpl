		...
		<tbody>
			{foreach $aArticleItems as $oArticleItem}
				<tr id="article-item-{$oArticleItem->getId()}">
					...
					<td class="ta-r">
						<a href="{$oAdminUrl->get('update')}{$oArticleItem->getId()}/" class="icon-edit" title="Изменить"></a>
						<a href="#" class="icon-remove" onclick="if (confirm('Действительно удалить?')) { ls.plugin.article.admin.removeArticle({$oArticleItem->getId()}); } return false;" title="Удалить"></a>
					</td>
				</tr>
			{/foreach}
		</tbody>

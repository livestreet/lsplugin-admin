...
class PluginAdmin_ActionAdmin_EventRbac extends Event {
	...
	public function EventPermissionList() {
		 ...
		 */
		$aPermissions=$this->Rbac_GetPermissionItemsByFilter(array('#order'=>array('code'=>'asc'),'#index-group'=>'group_id'));
	...
	public function EventGroupList() {
		...
		$aGroups=$this->Rbac_GetGroupItemsByFilter(array('#order'=>array('code'=>'asc'),'#with'=>array('permissions')));
	...
	public function EventRolePermissions() {
		 ...
		 * Получаем разрешения в разрезе групп
		 */
		$aPermissionsAll=$this->Rbac_GetPermissionItemsByFilter(array('#order'=>array('code'=>'asc'),'#index-group'=>'group_id'));
	...
	public function EventAjaxRolePermissionAdd() {
		...
		if (!$this->Rbac_GetRolePermissionByFilter(array('role_id'=>$oRole->getId(),'permission_id'=>$oPermission->getId()))) {
			/**
			 * Добавляем
			 */
			...
			if ($oRolePermission->Add()) {
				$this->Viewer_Assign('oPermission',$oPermission);
				$this->Viewer_Assign('oRole',$oRole);
				$this->Viewer_AssignAjax('sText',$this->Viewer_Fetch(Plugin::GetTemplatePath($this).'actions/ActionAdmin/rbac/role.permissions.item.tpl'));
			} else {
	...
	public function EventAjaxRolePermissionRemove() {
		...
		if ($oRel=$this->Rbac_GetRolePermissionByFilter(array('role_id'=>$iRoleId,'permission_id'=>$iPermissionId))) {
			$oRel->Delete();
		}
	}
	...
	public function EventRoleUsers() {
		 ...
		 */
		$aRoleUsers=$this->Rbac_GetRoleUserItemsByFilter(array('role_id'=>$oRole->getId(),'#index-from'=>'user_id','#page'=>array($iPage,20)));
		/**
		 * Формируем постраничность
		 */
		$aPaging=$this->Viewer_MakePaging($aRoleUsers['count'],$iPage,20,Config::Get('pagination.pages.count'),$oRole->getUrlAdminAction('users'));
		/**
	...
	public function EventAjaxRoleUserAdd() {
		...
		if (!$this->Rbac_GetRoleUserByFilter(array('role_id'=>$oRole->getId(),'user_id'=>$oUser->getId()))) {
			...
			if ($oRoleUser->Add()) {
				...
				$this->Viewer_Assign('oRole',$oRole);
				$this->Viewer_AssignAjax('sText',$this->Viewer_Fetch(Plugin::GetTemplatePath($this).'actions/ActionAdmin/rbac/role.users.item.tpl'));
			} else {
				$this->Message_AddError('Возникла ошибка при добавлении');
			}
		} else {
